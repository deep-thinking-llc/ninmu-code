...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".ninmu").join("CLAUDE.md"),
            dir.join(".ninmu").join("instructions.md"),
        ] {
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::write(
            root.join("apps").join(".ninmu").join("instructions.md"),
            "apps dot claude instructions",
        )
        .expect("write apps dot claude instructions");
        fs::write(nested.join(".ninmu").join("CLAUDE.md"), "nested rules")
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        ...
        let original_claw_home = std::env::var("NINMU_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("NINMU_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_claw_home {
            std::env::set_var("NINMU_CONFIG_HOME", value);
        } else {
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".ninmu/instructions.md")));
        assert!(
