...
/// fall back to reading `.ninmu-base` from `cwd`.
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("runtime-stale-base-{nanos}-{unique_id}"))
    }
    ...
    fn reads_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ninmu-base"), "abc1234def5678\n").expect("write .ninmu-base");
    ...
    fn returns_none_for_empty_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ninmu-base"), "  \n").expect("write empty .ninmu-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ninmu-base"), "from_file\n").expect("write .ninmu-base");
    ...
    fn matches_with_claw_base_file_in_real_repo() {
        ...
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".ninmu-base"), format!("{sha}\n")).expect("write .ninmu-base");
        let source = resolve_expected_base(None, &root);
    ...
    fn diverged_with_claw_base_file_after_new_commit() {
        ...
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".ninmu-base"), format!("{old_sha}\n")).expect("write .ninmu-base");
