...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    ...
    Ok(cwd.join(".nimmud-todos.json"))
}

fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    match ninmu_commands::resolve_skill_path(&cwd, skill) {
        Ok(path) => Ok(path),
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(ninmu_config_home) = std::env::var("NINMU_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&ninmu_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.ninmu/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".ninmu"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".ninmu"));
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".nimmud-agents"));
    }
    Ok(cwd.join(".nimmud-agents"))
}
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".ninmu").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("NINMU_CONFIG_HOME") {
...
#[allow(clippy::needless_pass_by_value)]
fn execute_powershell(input: PowerShellInput) -> std::io::Result<ninmu_runtime::BashCommandOutput> {
...
fn execute_shell_command(
    ...
    timeout: Option<u64>,
    run_in_background: Option<bool>,
) -> std::io::Result<ninmu_runtime::BashCommandOutput> {
    if run_in_background.unwrap_or(false) {
        let child = std::process::Command::new(shell)
            ...
            .spawn()?;
        return Ok(ninmu_runtime::BashCommandOutput {
            stdout: String::new(),
            stderr: String::new(),
            raw_output_path: None,
            interrupted: false,
...
Command exceeded timeout of {timeout_ms} ms",
    ...
    Ok(ninmu_runtime::BashCommandOutput {
        stdout: String::from_utf8_lossy(&output.stdout).into_owned(),
        stderr: String::from_utf8_lossy(&output.stderr).into_owned(),
        raw_output_path: None,
        interrupted: false,
...
mod tests {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .ninmu/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let ninmu_dir = worktree.join(".ninmu");
        fs::create_dir_all(&ninmu_dir).expect("create .ninmu dir");
        ...
        fs::write(ninmu_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .ninmu/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".ninmu").join("worker-state.json");
    ...
    fn runtime_tools_extend_registry_definitions_permissions_and_search() {
        ...
        let search = registry.search(
            ...
            Some(ninmu_runtime::McpDegradedReport::new(
                vec!["demo".to_string()],
                vec![ninmu_runtime::McpFailedServer {
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".ninmu").join("skills").join("plan");
        let command_dir = root.join(".ninmu").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ninmu/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ninmu/commands/handoff.md"));
    ...
    impl ninmu_runtime::ApiClient for MockSubagentApiClient {
        fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
            self.calls += 1;
            match self.calls {
                1 => {
    ...
    fn subagent_runtime_executes_tool_loop_with_isolated_session() {
        ...
        assert!(runtime
            ...
            .any(|block| matches!(
                block,
               ninmu_runtime::ContentBlock::ToolResult { output, .. }
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".ninmu")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".ninmu")).expect("cwd dir");
        std::fs::write(
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".ninmu")).expect("cwd dir");
        std::fs::write(
            cwd.join(".ninmu").join("settings.local.json"),
        ...
        let local_settings =
            std::fs::read_to_string(cwd.join(".ninmu").join("settings.local.json"))
                .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".ninmu").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn read_only_registry() -> super::GlobalToolRegistry {
        use ninmu_runtime::permission_enforcer::PermissionEnforcer;
        use ninmu_runtime::PermissionPolicy;

        let policy = mvp_tool_specs().into_iter().fold(
            PermissionPolicy::new(ninmu_runtime::PermissionMode::ReadOnly),
            |policy, spec| policy.with_tool_requirement(spec.name, spec.required_permission),
        );
    ...
    fn run_task_packet_creates_packet_backed_task() {
        use ninmu_runtime::task_packet::TaskScope;
